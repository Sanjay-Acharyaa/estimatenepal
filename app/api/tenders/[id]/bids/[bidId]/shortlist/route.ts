...
export async function POST(request: NextRequest, { params }: Params): Promise<NextResponse> {
  try {
    ...
    if (!procurementRoles.includes("CLIENT")) {
      return apiError("FORBIDDEN", "Only clients can shortlist bids.", 403);
    }
    ...
    if (bid.status !== "SUBMITTED") {
      return apiError("CONFLICT", `Only submitted bids can be shortlisted. Current status: ${bid.status}.`, 409);
    }
