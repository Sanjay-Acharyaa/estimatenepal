...
export function verificationEmailHtml(url: string, name: string) {
  ...
  return `
    <div style="font-family:sans-serif;max-width:500px;margin:auto">
      ...
      <a href="${safeUrl(url)}" style="display:inline-block;padding:12px 24px;background:#2563eb;color:#fff;border-radius:6px;text-decoration:none;font-weight:bold">Verify Email</a>
...
export function inviteEmailHtml(inviteUrl: string, orgName: string, inviterName: string, role: string) {
  ...
  return `
    <div style="font-family:sans-serif;max-width:500px;margin:auto">
      ...
      <p><strong>${safeInviterName}</strong> has invited you to join <strong>${safeOrgName}</strong> as a <strong>${safeRole}</strong>.</p>
      <p>Click the button below to accept. This invitation expires in 72 hours.</p>
      <a href="${safeUrl(inviteUrl)}" style="display:inline-block;padding:12px 24px;background:#2563eb;color:#fff;border-radius:6px;text-decoration:none;font-weight:bold">Accept Invitation</a>
  ...
  return `
    <div style="font-family:sans-serif;max-width:560px;margin:auto;color:#111827">
      ...
      <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;padding:32px;border-radius:0 0 12px 12px">
        <h2 style="font-size:18px;margin:0 0 12px">Project Proposal: ${safeProjectName}</h2>
        <p style="color:#6b7280;margin:0 0 24px"><strong>${safeSenderName}</strong> from <strong>${safeOrgName}</strong> has shared a project proposal with you.</p>
        ...
        <a href="${safeUrl(shareUrl)}" style="display:inline-block;padding:14px 28px;background:#2563eb;color:#fff;border-radius:8px;text-decoration:none;font-weight:bold;font-size:15px">Review Proposal →</a>
...
export function passwordResetEmailHtml(url: string) {
  return `
    <div style="font-family:sans-serif;max-width:500px;margin:auto">
      ...
      <a href="${safeUrl(url)}" style="display:inline-block;padding:12px 24px;background:#2563eb;color:#fff;border-radius:6px;text-decoration:none;font-weight:bold">Reset Password</a>
