...
| BL-015 | 2026-08-29 | Bidding Platform SSO link in dashboard sidebar opens a new tab rather than integrating inline. Once I-5 is complete and tender routes exist within Estimation, this link should change to a direct internal nav link to `/tenders` rather than an SSO redirect. | I-5 |
| BL-016 | 2026-08-29 | Notification bell in Estimation dashboard is present but only handles canvas collaboration events. It does not show procurement notifications. Needs extending in I-4 and I-9. | I-4 |
| BL-017 | 2026-08-29 | When Bidding code is copied into Estimation, all `getServerUser()` calls (Bidding auth function) must be replaced with `getSession()` (Estimation auth function) before any copied route is testable. This is a systematic find-and-replace across all I-5 through I-10 copied files. | I-5 |
| BL-018 | 2026-08-29 | Post-login redirect is always `/dashboard`. Once I-5 ships the `/tenders` route, users with procurementRoles of CLIENT or CONTRACTOR should land on `/tenders` instead of `/dashboard`. Implement in I-5 by reading the session's procurementRoles after signIn resolves. | I-5 |
...
| BL-021 | 2026-08-29 | `dispatchUserNotification` removed from `bulk-approve` and `bulk-reject` routes — the function (`lib/bid-notifications`) does not exist in Estimation. When procurement notifications are built (I-4), re-add notification dispatch in these routes. | I-4 |
| BL-022 | 2026-08-29 | Landing page stats strip (active tenders, registered contractors, districts covered) not yet live — requires `/api/public/stats` route which doesn't exist until I-5. Add the strip and API route in I-5 after tender models are in place. | I-5 |
| BL-023 | 2026-08-29 | Landing page open tenders preview (3 tender cards) not yet live — requires `/api/tenders/search` and the `/tenders` route which are I-5. Add the preview section and its API route in I-5. | I-5 |
